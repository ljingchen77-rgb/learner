...
function loadPost(file){if(!/^posts\/[\w.-]+\.md$/.test(file))return showStatus(itemsContainer,'文章地址无效。');listView.classList.add('hidden');contentView.classList.add('active');scrollTo(0,0);showStatus(bodyContainer,'正在读取文章…');fetch(file).then(response=>{if(!response.ok)throw new Error('文章加载失败');return response.text()}).then(markdown=>{if(!window.marked||!window.DOMPurify)throw new Error('安全渲染组件加载失败，请刷新后重试');const title=markdown.split('\n')[0].replace(/^#\s*/,'');const rendered=marked.parse(markdown);bodyContainer.innerHTML=DOMPurify.sanitize(rendered);document.title=`${title} · 刘靖臣`;renderArticleTools(file,markdown);loadComments()}).catch(error=>showStatus(bodyContainer,`⚠ ${error.message}`))}
